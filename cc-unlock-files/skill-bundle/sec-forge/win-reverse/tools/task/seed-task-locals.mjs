...
const quickAdoptionPlan = [
  {
    slug: "pe-triage-unpack-network",
    topics: ["static-triage", "packer-unpack", "tls-network"]
  },
  {
    slug: "anti-analysis-ui",
    topics: ["anti-analysis", "ui-runtime"]
  },
  {
    slug: "dotnet-ui-runtime",
    topics: ["dotnet", "ui-runtime"]
  },
  {
    slug: "driver-ioctl-network",
    topics: ["driver", "tls-network", "static-triage"]
  },
  {
    slug: "triage-static-driver",
    topics: ["static-triage", "driver"]
  },
  {
    slug: "loader-config-network",
    topics: ["loader-injection", "config-recovery", "tls-network"]
  },
  {
    slug: "triage-loader-ui",
    topics: ["static-triage", "loader-injection", "ui-runtime"]
  }
];
...
const explicitFieldDefaults = {
  "staticTriage.status": "captured",
  "packerUnpack.status": "captured",
  "antiAnalysis.status": "captured",
  "dotnetAnalysis.status": "captured",
  "driverAnalysis.status": "captured",
  "tlsNetwork.status": "captured",
  "uiRuntime.status": "captured"
};
...
function configureTask(taskDir, topicSpecs, marker) {
  ...
  task.targetContext.targetProcessNames = Array.from(
    new Set([...(task.targetContext.targetProcessNames || []), `${topics[0] || "task"}-host.exe`])
  );
  task.targetContext.targetBinaryPath = task.targetContext.targetBinaryPath || `C:/seed/${topics[0] || "task"}.exe`;
  task.targetContext.targetNetworkIndicators = Array.from(
    new Set([...(task.targetContext.targetNetworkIndicators || []), `${topics[0] || "task"}.seed.local:443`])
  );
...
function main() {
  ...
  if (!options.dryRun) {
    console.log("[seed] next: npm run qa:dashboard");
  }
}
