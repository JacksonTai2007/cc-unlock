...
function upsertMarkdownSection(markdown, headingLine, body) {
  ...
  for (const section of sections) {
    if (section.headingLine.trim() === headingLine.trim()) {
      return `${source.slice(0, section.headingStart)}${replacement}${source.slice(section.end).replace(/^\s*/, "\n")}`;
    }
  }
...
function syncCloseoutReport(taskDir, task, routeState, options = {}) {
  ...
  const phase = cleanText(task.phase) || cleanText(routeState.phase) || "Close";
...
export function synchronizeCloseoutState(taskDir, taskInput, options = {}) {
  ...
  routeState.phase = completedPhase;
  ...
  task.phase = completedPhase;
  task.routeState.stateFreshnessStatus = freshness.ok ? "fresh" : "stale-was-detected-before-closeout-sync";
  task.routeState.lastFreshnessCheckAt = completedAt;
