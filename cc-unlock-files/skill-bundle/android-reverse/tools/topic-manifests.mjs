...
const maturityLabels = {
  "synthetic-e2e": "已具备 registry-backed synthetic task 骨架、formal validation 与专题产物约束；该级别只代表仓库级 synthetic 闭环，不等同于所有真实目标都已完成实战回归。",
  "closed-loop": "已具备 registry-backed task model、formal validation 与专题产物约束，但未声明 synthetic task pack 保证。",
...
function normalizeTaskInit(taskInit = {}) {
  ...
  const baseProtectionTier = String(taskInit.baseProtectionTier || "A0").trim() || "A0";
...
export function renderCapabilityMatrix(topics = readTopicRegistryFromSource()) {
  ...
  for (const topic of sortTopics(topics)) {
    ...
    lines.push(
      `- taskInit: aliases=${formatList(topic.taskInit?.aliases || [])}, baseProtectionTier=\`${topic.taskInit?.baseProtectionTier || "A0"}\`, combinationProtectionTiers=${formatCombinationProtectionTiers(topic.taskInit?.combinationProtectionTiers || [])}`
    );
